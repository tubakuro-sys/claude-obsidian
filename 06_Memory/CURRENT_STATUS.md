...
| Frostlight（凍てつく灯火） | 🔧 プロトタイプ完成・改修フェーズ | 第一段階「経済の嘘を消す」：リングの断熱を環ごとに |
| 結婚式BGM DVD作成 | 🔧 制作中 | 購入CDをMP3リッピング→シーン別フォルダ→マスター形式でDVD書込 |
...
**並行・引き継き**
- [ ] 結婚式BGM：購入CDをMP3リッピング→シーン別（入場/中座等）に振り分け→データDVDに書込み（式場スタッフ提出用）
- [ ] calorie_app 3品表示バグ修正（Console確認でき次第）
- [ ] 習慣トラッカー v1設計・実装開始
- [ ] Linuxゲートウェイ永続化（30分）
- [ ] Android運用改善（ブックマーク整理）
- [ ] 家族共有アプリ：Supabaseセットアップ→schema.sql実行→雛形作成（フェーズ1）
- [ ] Frostlight 第一段階：リングの断熱をその環に住む者だけに効かせる

---

## ⚡ 直近の決定事項（2026-07-20更新）

- **2026-07-20：結婚式BGMを自作DVDで式場に提出する方針決定**
  - 式場から「購入CD・iTunes等の音源を複製できる形式でDVD提出」と指定→著作権処理は式場側に任せてよい（ISUM申請不要、自分では素材ファイルを用意するだけ）
  - 音声形式：式場指定を失念 → とりあえず **MP3（320kbps）** で作成してみる
  - 作り方：購入CDをWMPでMP3リッピング→シーン別フォルダ（入場/歓談/中座/再入場/退場）に振り分け・リネーム（シーン名を先頭に）→空DVD-Rに「マスター形式」で書込み
  - ⚠️ iTunesの「購入曲」はDRMなしでコピー可、「Apple MusicサブスクのDL曲」はDRM保護でコピー不可（混在注意）
  - シーンごとの曲順リスト（フル/フェードアウト等）をメモで一緒に渡すと当日スムーズ

- **2026-07-10：Frostlight（凍てつく灯火）プロトタイプ完成・技術方針決定**
  - オリジナルのターン制サバイバル都市経営ゲーム（単一HTML）。70日／4章／難易度4段階
  - **エンジン移植はしない**。今のHTML/CSS/JSのまま進め、配布は Tauri でラップ
    - 移植に2ヶ月かかるが弱点（経済の嘘・選択の浅さ）はエンジンを変えても直らない。9割がUIなのでHTMLが最速
  - Unity Personal は 20万USD 未満なら無料・ロイヤリティなし（Runtime Fee撤回済み）。ただし Switch/PS/Xbox は Pro 必須
  - Godot は完全無料・収益制限なし・2D/UIが得意（将来の選択肢）
  - **方針：第一段階「経済の嘘を消す」→ 第二段階「手触り（音→間→絵）」→ 第三段階「Tauriで包む」**
  - 第一段階では絵も音も一切追加しない。灰色の四角のまま面白いか判定する
  - 詳細は `02_Projects/Frostlight.md` / `04_Logs/Daily/2026-07-10.md` に記録
